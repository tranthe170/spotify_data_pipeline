...
class UserUtils:
    ...
    def generate_users_data(location_mapping_path: str, n_users: int) -> pd.DataFrame:
        """function to generate random users demographics data.
        ...
        users_df["first_name"] = [fake.first_name_male() if gender == "male" else fake.first_name_female() for gender in genders]
        users_df["last_name"] = [fake.last_name() for i in range(n_users)]
