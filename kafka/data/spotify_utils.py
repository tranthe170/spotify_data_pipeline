...
class SpotifyUtils:
    ...
    def get_audio_features(self, track_ids: List[str]) -> pd.DataFrame:
        """Get audio features for every track ids
        ...
        if track_ids:
            ...
            for iteration in range(iterations):
                ...
                for af in audio_features:
                    if af is None:
                        dfs_list.append(pd.Series([np.nan]*len(required_features), index=required_features))
                    else:
                        dfs_list.append(pd.Series(af)[required_features])
            df = pd.concat(dfs_list, axis=1).transpose().reset_index(drop=True)
    ...
    def generate_songs_data(self, playlist_uri: List[str]) -> pd.DataFrame:
        ...
        # remove song have audio features missing
        song_df = song_df.dropna(subset=["danceability"])
